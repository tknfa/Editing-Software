...
class AddToTimeline(QDialog):
    ...
    def __init__(self, files=None, position=0.0, track_num=None):
        # Create dialog class
        ...
        if track_num is not None:
            for index in range(self.cmbTrack.count()):
                if self.cmbTrack.itemData(index) == track_num:
                    self.cmbTrack.setCurrentIndex(index)
                    break
