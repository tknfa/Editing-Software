...
def _sanitize_button_text(text):
    """Backward-compatible wrapper for widget-safe text."""
    return sanitize_ui_text(text)
...
class PropertiesTableView(QTableView):
    ...
    def focus_property(self, property_name):
        """Select and focus a property row by its internal property key."""
        model = self.clip_properties_model.model
        for row in range(model.rowCount()):
            ...
            property_data = label_item.data()
            if not property_data or property_data[0] != property_name:
                continue

            self.selected_label = label_item
            self.selected_item = value_item
            self.property_type = property_data[1]["type"]
            self.selectRow(row)
            self.setCurrentIndex(value_item.index())
            self.scrollTo(value_item.index(), QAbstractItemView.PositionAtCenter)
...
class SelectionLabel(QFrame):
        ...
        try:
            for selected in selection:
                ...
                if item_type == "clip":
                    ...
                    action = menu.addAction(thumb_icon, sanitize_ui_text(item_name))
                    ...
                    for effect_info in clip.data.get('effects', []):
                        ...
                        effect_action = menu.addAction(
                            effect_icon,
                            sanitize_ui_text('  >  %s' % _(effect_name)),
                        )
                    ...
                    action = menu.addAction(item_icon, sanitize_ui_text(item_name))
    ...
    def select_item(self, selection):
        ...
        if self.item_type == "multi":
            ...
            self.btnSelectionName.setText(_sanitize_button_text(_("%d selections") % count))
        ...
        if self.item_id:
            self.lblSelection.setText("<strong>%s</strong>" % _("Selection:"))
            self.btnSelectionName.setText(_sanitize_button_text(self.item_name))
            self.btnSelectionName.setVisible(True)
            if self.item_icon:
